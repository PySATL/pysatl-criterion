...
class KSNormalityTest(AbstractNormalityTestStatistic, KSTestStatistic):
    ...
    def code():
        return (
            ...
            + "_"
            + super(
                AbstractNormalityTestStatistic, AbstractNormalityTestStatistic
            ).code()
        )
    ...
    def execute_statistic(self, rvs, **kwargs):
        ...
        return KSTestStatistic.execute_statistic(
            self, rvs, cdf_vals
        )  # TODO: should test it
...
class ADNormalityTest(AbstractNormalityTestStatistic, ADTestStatistic):
    ...
    def calculate_critical_value(
        self, rvs_size, sl, count=500_000
...
class SWNormalityTest(AbstractNormalityTestStatistic):
    ...
    def ordered_statistic(n):
        ...
        m = np.array(
            [scipy_stats.norm.ppf((i - 3 / 8) / (n + 0.25)) for i in range(1, n + 1)]
        )
...
class SkewNormalityTest(AbstractNormalityTestStatistic):
    ...
    def skew_test(a):
        ...
        if n < 8:
            raise ValueError(
                "skew test is not valid with less than 8 samples; %i samples were given."
                % int(n)
            )
...
class KurtosisNormalityTest(AbstractNormalityTestStatistic):
    ...
    def kurtosis_test(a):
        ...
        a = 6.0 + 8.0 / sqrt_beta1 * (
            2.0 / sqrt_beta1 + np.sqrt(1 + 4.0 / (sqrt_beta1**2))
        )
        term1 = 1 - 2 / (9.0 * a)
...
class EPNormalityTest(AbstractNormalityTestStatistic):
    ...
    def execute_statistic(self, rvs, **kwargs):
        ...
        a = np.sqrt(2) * np.sum(
            [np.exp(-((x[i] - x_mean) ** 2) / (4 * m2)) for i in range(n)]
        )
        b = 0
        for k in range(1, n):
...
class Hosking1NormalityTest(AbstractNormalityTestStatistic):
    ...
    def stat10(x):
        ...
        if n > 3:
            ...
            b1 = (
                1.0 * x_tmp[1] + 2.0 * x_tmp[2] + sum(i * x_tmp[i] for i in range(3, n))
            )
    ...
    def execute_statistic(self, rvs, **kwargs):
        ...
        if n > 3:
            ...
            for i in range(1, n + 1):
                stat_zc += (
                    np.log((1.0 / phiz[i - 1] - 1.0) / ((n - 0.5) / (i - 0.75) - 1.0))
                    ** 2
                )
            return stat_zc
...
class ZhangWuANormalityTest(AbstractNormalityTestStatistic):
    ...
    def execute_statistic(self, rvs, **kwargs):
        ...
        if n > 3:
            ...
            stat_za = 0.0
            for i in range(1, n + 1):
                stat_za += np.log(phiz[i - 1]) / ((n - i) + 0.5) + np.log(
                    1.0 - phiz[i - 1]
                ) / (i - 0.5)
            stat_za = -stat_za
...
class GlenLeemisBarrNormalityTest(AbstractNormalityTestStatistic):
    ...
    def execute_statistic(self, rvs, **kwargs):
        ...
        if n > 3:
            ...
            stat_ps = 0
            for i in range(1, n + 1):
                stat_ps += (2 * n + 1 - 2 * i) * np.log(phiz[i - 1]) + (
                    2 * i - 1
                ) * np.log(1 - phiz[i - 1])
            return -n - stat_ps / n
...
class CabanaCabana1NormalityTest(AbstractNormalityTestStatistic):
    ...
    def stat19(x):
        ...
        if n > 3:
            ...
            mean_h4 = np.sum(z_data**4 - 6 * z_data**2 + 3) / (
                2 * np.sqrt(6) * np.sqrt(n)
            )
            ...
            mean_h7 = np.sum(
                z_data**7 - 21 * z_data**5 + 105 * z_data**3 - 105 * z_data
    ...
    def stat20(x):
        ...
        if n > 3:
            ...
            for i in range(n):
                ...
                h5[i] = (
                    math.pow(z[i], 5.0) - 10.0 * math.pow(z[i], 3.0) + 15.0 * z[i]
                ) / (2.0 * np.sqrt(30.0))
                h6[i] = (
                    math.pow(z[i], 6.0)
...
class ChenShapiroNormalityTest(AbstractNormalityTestStatistic):
    ...
    def stat26(x):
        ...
        if n > 3:
            ...
            # mean_x = np.mean(x)
            var_x = np.var(x, ddof=1)
            m = scipy_stats.norm.ppf(
                np.arange(1, n + 1) / (n + 0.25) - 0.375 / (n + 0.25)
            )
            stat_cs = np.sum((xs[1:] - xs[:-1]) / (m[1:] - m[:-1])) / (
                (n - 1) * np.sqrt(var_x)
            )
    ...
    def stat27(x):
        ...
        if n > 3:
            ...
            for i in range(5, n - 3):
                b[i - 1] = (
                    1.0 / (u[i - 1] - u[i + 3]) - 1.0 / (u[i - 5] - u[i - 1])
                ) / (n - 4)
            q1 = np.dot(a, xs)
            q2 = np.dot(b, xs)
            stat_q = np.log(q1 / q2)
...
class CoinNormalityTest(AbstractNormalityTestStatistic):
    ...
    def stat30(self, x):
        ...
        if n > 3:
            ...
            stat_beta32 = (
                (term1 * term2 - term3 * term4) / (term1 * term1 - term3 * term6)
            ) ** 2
            return stat_beta32
...
class GMGNormalityTest(AbstractNormalityTestStatistic):
    ...
    def stat33(x):
        ...
        if n > 3:
            ...
            pi = 4.0 * math.atan(
                1.0
            )  # or use pi = M_PI, where M_PI is defined in math.h
...
class BHSNormalityTest(AbstractNormalityTestStatistic):
    ...
    def mc_c_d(self, z, eps, iter_):
        ...
        for i in range(n):
            zi = z[i]
            x[i + 1] = -(
                large if zi == float("inf") else (-large if zi == -float("inf") else zi)
            )
        ...
        if trace_lev:
            print(
                f"mc_C_d(z[1:{n}], trace_lev={trace_lev}): Median = {-x_med} (not at the border)"
            )
        ...
        if trace_lev >= 2:
            print(
                f"   x1[] := {{x | x_j > x_eps = {x_eps}}}    has {j - 1} (='j-1') entries"
            )
        i = 1
        ...
        while not is_found and (nr - nl + neq > n) and it < iter_[0]:
            ...
            for i in range(h2, 0, -1):
                while (
                    j <= h1
                    and self.h_kern(x[j], x2[i - 1], j, i, h1 + 1, eps[1]) - trial
                    > eps_trial
                ):
                    j += 1
                p[i] = j - 1
            ...
            for i in range(1, h2 + 1):
                while (
                    j >= 1
                    and trial - self.h_kern(x[j], x2[i - 1], j, i, h1 + 1, eps[1])
                    > eps_trial
                ):
                    j -= 1
                q[i] = j + 1
            ...
            if knew <= sum_p:
                ...
                if trace_lev >= 3:
                    print(
                        "; s_p < kn ?<=? s_q: {}".format("TRUE" if is_found else "no")
                    )
                if is_found:
                    med_c = trial
                else:  # knew > sum_q
        ...
        if not converged:
            print(
                f"maximal number of iterations ({iter_[0]} =? {it}) reached prematurely"
            )
            # still:
            med_c = trial
    ...
    def stat35(x):
        ...
        if n > 3:
            ...
            rn = n * (
                ...
                + (-r1 * 32040.6956903 + r2 * 918649.9005906 - r3 * 2425883.3443201)
                * r2
                + (r1 * 85065.7773947 - r2 * 2425883.3443201 + r3 * 6407749.8211208)
                * r3
            )
...
class GraphEdgesNumberNormTest(
    AbstractNormalityTestStatistic, GraphEdgesNumberTestStatistic
):
    @staticmethod
    @override
    def code():
...
class GraphMaxDegreeNormTest(
    AbstractNormalityTestStatistic, GraphMaxDegreeTestStatistic
):
    @staticmethod
    @override
    def code():
