# Graph edges number test for gamma distribution

## Description
Performs graph edges number goodness-of-fit test for the hypothesis that the sample comes from a gamma distribution.
The statistic counts edges in a proximity graph built after transforming observations with the gamma cumulative distribution function.
...
test_statistic = GraphEdgesNumberGammaGofStatistic(alpha=2, beta=1)
