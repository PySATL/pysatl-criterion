# Graph average degree test for gamma distribution

## Description
Performs graph average degree goodness-of-fit test for the hypothesis that the sample comes from a gamma distribution.
The statistic computes the mean vertex degree in a proximity graph built on gamma CDF transformed observations.
...
test_statistic = GraphAverageDegreeGammaGofStatistic(alpha=2, beta=1)
...
## Details
The sample is transformed to gamma CDF values. A proximity graph is built on the transformed values, and the statistic is the average vertex degree.
