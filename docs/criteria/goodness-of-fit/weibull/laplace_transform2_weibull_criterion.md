# Laplace transform 2 test for Weibull distribution

## Description
Performs Laplace transform 2 goodness-of-fit test for the hypothesis that the sample comes from a Weibull distribution.
...
Test Statistic
The statistic is based on a squared-distance statistic based on the empirical Laplace transform over type-2 integration points.
...
test_statistic = LaplaceTransform2WeibullGofStatistic(a=1, k=2)
statistic_result = test_statistic.execute_statistic([0.42, 0.65, 0.88, 1.12, 1.43, 1.76, 2.05, 2.44, 2.91, 3.37], m=20, a=-5)
