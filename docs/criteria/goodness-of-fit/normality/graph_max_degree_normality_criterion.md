# Graph max degree test for normality

## Description
Performs the Graph max degree goodness-of-fit test for the hypothesis of normality.
...
Test Statistic
The statistic is based on the maximum degree in a proximity graph built from the sample.
...
test_statistic = GraphMaxDegreeNormalityGofStatistic()
...
## Details
The implementation evaluates the Graph max degree statistic for the supplied observations. Large or small values should be interpreted according to the statistic alternative used by the class implementation.
